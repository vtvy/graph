...
//			//     System.out.print(StaticData.hashedEdges.hashCode());//throw an exception if more than one graph was created
		}
		...
		if(Settings.multipleAtts)
		{
			...
			if(Settings.multipleAtts)
				for (Iterator<Entry<String, Integer>> iterator = multiAttLabels.entrySet().iterator(); iterator.hasNext();) 
				{
					...
					//     System.out.print("MultiAtt "+entry.getKey()+" with label "+entry.getValue()+" and size "+nodesByLabel.get(entry.getValue()).size());	
				}
	...
	public void printFreqNodes()
	{
		for (Iterator<  java.util.Map.Entry< Integer, HashMap<Integer,myNode> > >  it= freqNodesByLabel.entrySet().iterator(); it.hasNext();) 
		{
			...
			//     System.out.print("Freq Label: "+ar.getKey()+" with size: "+ar.getValue().size());
		}
	}

...
public class GSpanExtender<NodeType, EdgeType> extends
    ...
    public void call(final SearchLatticeNode<NodeType, EdgeType> node,
        ...
        //Tính support nhưng không kiểm tra điều kiện support, in tất cả 
        for (final Extension<NodeType, EdgeType> ext : extensions) {
            //calculate support
            SearchLatticeNode<NodeType, EdgeType> newNode = node.extend(ext);
//            Frequency childFreq = ((Frequented) newNode).frequency();
...
//            boolean isSatifyMinSub = freqThreshold.compareTo(childFreq) <= 0;
            // if >= minsub then add into children
//            if (isSatifyMinSub) {
                children.add(newNode);
                ((GSpanExtension<NodeType, EdgeType>) ext).release();
//            } 
...
//            else {
//                System.out.println("Tap con bi loai bo co support la: " + childFreq);
//                System.out.println("Va minsub luc nay la: " + freqThreshold);
//                String out = DFScodeSerializer.serialize(((DFSCode<NodeType, EdgeType>) newNode).me);
...
//            }
     ...
     */
    public Collection<SearchLatticeNode<NodeType, EdgeType>> getChildren(
            final SearchLatticeNode<NodeType, EdgeType> node, IntFrequency freqThreshold) {

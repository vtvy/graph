...
public class RightMostExtension<NodeType, EdgeType> extends
    ...
    protected void add(final GSpanEdge<NodeType, EdgeType> gEdge,
        ...
        //check for edge possibility; this is needed in GraMi only because we extend with all combinations, it is needed to prune some combinations gEdge.
        // search corresponding extension
        GSpanExtension<NodeType, EdgeType> ext = children.get(gEdge);
        //~

        if (ext == null) {
            //make sure that no impossible extension is applied
            ...
            ext.edge = gEdge;
            ext.frag = new DFSCode<NodeType, EdgeType>(code.getSortedFreqLabels(), code.getFreqEdgeLabels(), code.getSingleGraph(), utilities.Util.clone(code.getNonCandidates())).set((HPListGraph<NodeType, EdgeType>) ng, code.getFirst(), code.getLast(), code.getParents());
    ...
    protected final void extend(final DFSCode<NodeType, EdgeType> code) {
        ...
        // find extensions of the last node;
        {
            ...
            for (int i = 0; i < pairs.size(); i++) {
                ...
                for (int j = 0; j < freqEdgeLabels.size(); j++) {
                    final GSpanEdge<NodeType, EdgeType> gEdge = new GSpanEdge<NodeType, EdgeType>().set(lastNode, lastNode + 1, sortedFreqLabels.indexOf(theLabelA), freqEdgeLabels.get(j).intValue(), index, 0, theLabelA, label);
            ...
            for (int i = 0; i < vrs.length; i++) {
                ...
                if (labelDC.contains(candidateVB.getLabel())) {
                    ...
                    for (int j = 0; j < freqEdgeLabels.size(); j++) {
                        final GSpanEdge<NodeType, EdgeType> gEdge = new GSpanEdge<NodeType, EdgeType>().set(lastNode, candidateVB.getID(), sortedFreqLabels.indexOf(theLabelA), freqEdgeLabels.get(j).intValue(), sortedFreqLabels.indexOf(theLabelB), 0, theLabelA, theLabelB);
                        if ((code.getLast().compareTo(gEdge) < 0)) {
                            add(gEdge, code, 1);
                        } else {
                            gEdge.release();
        ...
        do {
            ...
            for (int i = 0; i < pairs.size(); i++) {
                ...
                int index = currentPoint.y;
                for (int j = 0; j < freqEdgeLabels.size(); j++) {
                    final GSpanEdge<NodeType, EdgeType> gEdge = new GSpanEdge<NodeType, EdgeType>().set(ackNode, lastNode + 1, sortedFreqLabels.indexOf(theLabelA), freqEdgeLabels.get(j).intValue(), index, 0, theLabelA, label);
                    add(gEdge, code, 0);
                }
            }
        } while (ackNode > 0);
